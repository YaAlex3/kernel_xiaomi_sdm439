...
/*
 * read_eeprom_info_for_gc5035() - read map data into buffer
 ...
 */
static int read_eeprom_info_for_gc5035(struct msm_eeprom_ctrl_t *e_ctrl,
...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (e_ctrl->userspace_probe == 0) {
		...
		rc = read_eeprom_info_for_gc5035(e_ctrl, &e_ctrl->cal_data);
		if (rc < 0) {
			pr_err("%s read_eeprom_info_for_gc5035 failed\n", __func__);
			goto power_down;
		}
